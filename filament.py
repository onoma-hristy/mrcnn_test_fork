...
def create_db(conn):
	try:
	    conn.execute('''CREATE TABLE IF NOT EXISTS FILE
	    			(FILE_NAME VARCHAR(100) NOT NULL, DIMENSION VARCHAR(20) NOT NULL, CAPTURE_DATE TIMESTAMP NOT NULL, SOLAR_DISK_X INT NOT NULL, SOLAR_DISK_Y INT NOT NULL, SOLAR_DISK_RADIUS INT NOT NULL, DETECTION_DURATION VARCHAR(50),  SEGMENTATION_DURATION VARCHAR(50), PRIMARY KEY (FILE_NAME))''')
	    conn.execute('''CREATE TABLE IF NOT EXISTS SOLAR_FEATURES
	    			(FILE_NAME VARCHAR(100) NOT NULL, BOUNDING_BOX VARCHAR(100) NOT NULL, CLASS VARCHAR(50) NOT NULL, MASK TEXT NOT NULL, PRIMARY KEY (FILE_NAME, BOUNDING_BOX), FOREIGN KEY (FILE_NAME) REFERENCES FILE(FILE_NAME))''')
    ...
    if r['masks'].shape[-1] > 0:
    	...
    	for i, x in enumerate(r['rois']):
    		if r['class_ids'][i] == 1: # 1 is filaments
    			...
    			binary_slices.append(masked_area[x[0]:x[2],x[1]:x[3]])
    			masked_slices.append(masked_area[x[0]:x[2],x[1]:x[3]])
    			original_slices.append(masked_area[x[0]:x[2],x[1]:x[3]])
    			skeleton.append(masked_area[x[0]:x[2],x[1]:x[3]])
...
def detect_filament(model, image_path=None):
    ...
    if image_path:
        ...
        conn = sqlite3.connect(results_dir+'/filament.db') 
        create_db(conn)        
        ...
        dduration = end_time-start_time #detection duration
        ...
        end_time = datetime.datetime.now()
        sduration = end_time-start_time #segmentation duration

        timestamp = datetime.datetime.strptime(filename[0], '%Y-%m-%d_%H-%M-%S')
        conn.execute("INSERT INTO FILE \
      		VALUES (?,?,?,?,?,?,?,?)",
      		[str(filename[0])+"."+str(filename[1]), str(image.shape), timestamp, c[0],c[1],c[2],str(dduration),str(sduration)]) 

        for e, sliced in enumerate(binary[2]):
        	object_bbox = json.dumps(r['rois'][e].tolist())
        	sliced_mask = np.zeros_like(binary[3][e])
        	sliced_mask = np.where(binary[3][e]==255,255,0)
        	sliced_flat=sliced_mask.flatten()
        	maskindex = np.where(sliced_flat == 0)
        	masklist = json.dumps(maskindex[0].tolist())
        	
        	conn.execute("INSERT INTO SOLAR_FEATURES \
      			VALUES (?,?,?,?)", [str(filename[0])+"."+str(filename[1]), 
      			object_bbox, int(r['class_ids'][e]), masklist])        	
        	if r['class_ids'][e] == 1:
        ...
        tduration = total_duration_end -total_duration_start
        print(tduration)
        conn.close() 
